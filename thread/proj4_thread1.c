...
void * user(void *para){
	...
	for(index=0;index<200;index++){
		...
		printf("user %d allocatedNewInt returns: %d\n", (*userid), indexes[index]);
	...
	sem_wait(&print);
	//printf("user %d\n",(*userid));
	int *value=accessIntPtr(indexes[0]);
	printf("user %d, value is %d \n",(*userid),(*value));
	print_ram();
	//unlockMemory(indexes[0]);
	sem_post(&print);
...
int main(){
	...
	for(i=0;i<6;i++){
		pthread_create(&users[i],NULL,user,(void *)id+i);
	}
	for(i=0;i<6;i++){
